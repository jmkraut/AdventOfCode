...
class D4P2 {
    public static void main(String[] args) throws FileNotFoundException, IOException{
        ...
        boolean[] hasMatriceWon = new boolean[matrices];
                ...
                for(int matrix = 0; matrix < matrices; matrix++) {
                    for(int row = 0; row < rows; row++) {
                        ...
                        if(colTruthCounter > 4) {
                            hasMatriceWon[matrix] = true;
                            int falseCounter = 0;
                            int lastMatrixWinner = 0;
                            for(int i = 0; i < hasMatriceWon.length; i++) {
                                if(hasMatriceWon[i] == false){
                                    falseCounter++;
                                    lastMatrixWinner = i;
                                }
                            }
                            if(falseCounter == 1) {
                                winningBingoMatrix = lastMatrixWinner;
                                winningBingoNumber = bingoNumsIntArray[round + 1];
                                break mainLoop;
                            }
                        }
                    }
        ...
        System.out.println("Winning bingo total was: " + (winningTotal - winningBingoNumber) * winningBingoNumber);
        System.out.println("Winning bingo Matrix was: " + winningBingoMatrix);
        System.out.print("Winning bingo Number was: " + winningBingoNumber);
    }
}